...
module.exports = async (client, message) => {
  ...
  if (message.channel.type !== "dm") {
    ...
    if (guild_setup == true) {
      await axios
        .get(
          `${client.config.firebase_url}/whitelisted/${message.guild.owner.id}.json`
        )
        .then(function (response) {
  ...
  if (
    ((guild_setup == false && command !== "bind") ||
      owner_whitelisted == false) &&
    ...
    var badEmbed = new Discord.MessageEmbed()
      .setColor(0xf54242)
      .setDescription(
        `Sorry ${message.author}, but either <@${message.guild.owner.id}> isn't whitelisted or this guild hasn't been setup yet!\nSetup - \`!bind groupID\`\n\n**[If you have any questions, feel free to click here](https://discord.gg/fHpfmy5)**`
      );
    return message.channel
      .send(badEmbed)
      .then((message) =>
        message.delete({ timeout: 5000, reason: "delete error message" })
      );
  }
