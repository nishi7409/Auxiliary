...
exports.run = async (client, message, args) => {
  ...
  var ownerEmbed = new Discord.MessageEmbed()
    ...
    .addField(
      `\`${client.config.prefix}unlist <@mention>\``,
      `Whitelists a user to the database`
    );
  ...
  var officerEmbed = new Discord.MessageEmbed()
    ...
    .addField(
      ...
      `Adds # ${client.config.experience_name} to all profiles of provided usernames`
    )
    .addField(
      `\`${client.config.prefix}remove # username1, username2, etc...\``,
      `Removes # ${client.config.experience_name} from all profiles of provided usernames`
    );
  ...
  var globalEmbed = new Discord.MessageEmbed()
    ...
    .addField(
      ...
      `Pulls up information about the bot and sends a link to the repository over direct messages`
    )
    .addField(
    ...
    .addField(
      ...
      `Displays a link for users to invite the bot into their servers`
    )
    .addField(
      `\`${client.config.prefix}verify rblx_username\``,
      `Verifies your ROBLOX account (rblx_username) with your Discord account--simply makes sure you're not a robot`
    )
    .addField(
      `\`${client.config.prefix}view rblx_username\``,
      `View the profile of the given username (rblx_username)`
    );
