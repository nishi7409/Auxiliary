...
exports.run = async (client, message, args, groupID) => {
  ...
  if (
    !message.member.roles.cache.some(
      (role) => role.name === `${client.config.officer_role}`
    )
  ) {
    return message.channel.send(
      `Sorry ${message.author}, but only users with the **${client.config.officer_role}** role can run that command!`
    );
  }
  ...
  if (flag == false) {
    var badEmbed = new Discord.MessageEmbed()
      .setColor(0xf54242)
      .setDescription(
        `You must verify yourself before you can run the **add** command!`
      );
  ...
  if (
    ...
    var badEmbed = new Discord.MessageEmbed()
      .setColor(0xf54242)
      .setDescription(
        `You must specify a number (1-${client.config.max_experiencePoints}) for me to remove ${client.config.experience_name} points from the specified users\n\n**${client.config.prefix}remove # username1, username2, etc**`
      );
    return message.reply(badEmbed);
  }
  ...
  if (!args[2]) {
    var badEmbed = new Discord.MessageEmbed()
      .setColor(0xf54242)
      .setDescription(
        `Please provide the ROBLOX username that you want to remove ${client.config.experience_name} from\n\n**${client.config.prefix}remove # username1, username2, etc**`
      );
    return message.reply(badEmbed);
  }

  // collect usernames into an array
  var userArray = message.content
    .slice(message.content.indexOf(message.content.split(" ")[2]))
    .split(",");
  ...
  var workinEmbed = new Discord.MessageEmbed()
    .setImage(
      "https://media.tenor.com/images/334cf1e2aa89a90a274f5a4040d1a6ec/tenor.gif"
    )
  ...
  await message.channel
    .send(workinEmbed)
    .then((message) =>
      message.delete({ timeout: 4000, reason: "delete workin message" })
    );
  ...
  for (i = 0; i < userArray.length; i++) {
    ...
    await axios
      .get(
        `https://api.roblox.com/users/get-by-username?username=${rblx_username}`
      )
      .then(function (response) {
    ...
    while (flag) {
      ...
      for (not_i = 0; not_i < roles.length; not_i++) {
        if (
          roles[not_i].Rank == current_rolesetID &&
          current_rolesetID !== 255
        ) {
      ...
      if (previous_rolesetID >= 1) {
        ...
        await axios
          .get(
            `${client.config.firebase_url}/guilds/${message.guild.id}/role_xp/${previous_rolesetID}.json`
          )
          .then(function (response) {
        ...
        if (previousRank_xp !== -1) {
          if (new_total_points <= previousRank_xp) {
            await rblxFunctions.setRank({
              group: groupID,
              target: rblx_id,
              rank: previous_rolesetID,
            });
            var promotionEmbed = new Discord.MessageEmbed()
              .setColor(0xf54242)
              .setImage("https://media.giphy.com/media/qQdL532ZANbjy/giphy.gif")
              .setDescription(
                `**:confetti_ball: \`${rblx_username}\` has been demoted to \`${previous_rolesetName}\`! :confetti_ball:**`
              );
            ...
            await message.channel
              ...
              .then((message) =>
                message.delete({
                  timeout: 5000,
                  reason: "delete annoying promotion image embed",
                })
              );
          } else {
            flag = false;
