...
exports.run = async (client, message, args) => {
  if (message.author.id !== client.config.owner_id)
    return message.channel.send(
      `Sorry ${message.author}, but only the owner of this bot can run that command!`
    );
  ...
  await axios
    .get(`${client.config.firebase_url}/whitelisted/${discord_user_id}.json`)
  ...
  if (!in_db || !status_in_db) {
    ...
    if (!in_db) {
      ...
      await axios
        .get(
          `${client.config.firebase_url}/verified_users/${discord_user_id}.json`
        )
        .then(function (response) {
          if (response.data == null) {
      ...
      if (verifiedStatus == false) {
        return message.channel.send(
          `<@${discord_user_id}> **must** verify themself first before the user can be whitelisted!`
        );
      }

      await axios
        .get(`${client.config.firebase_url}/default_settings/client_total.json`)
        .then(function (response) {
