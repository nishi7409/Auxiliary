...
exports.run = async (client, message, args) => {
  ...
  if (message.author.id !== message.guild.owner.id)
    return message.channel.send(
      `Sorry ${message.author}, but only the guild owner can the **bind** command!`
    );
  ...
  await axios
    .get(
      `${client.config.firebase_url}/verified_users/${message.author.id}.json`
    )
  ...
  if (verified_status == false) {
    var badEmbed = new Discord.MessageEmbed()
      .setColor(0xf54242)
      .setDescription(
        `Sorry ${message.author}, but you must verify yourself before you bind this guild with a group!`
      );
  ...
  await axios
    .get(`${client.config.firebase_url}/guilds/${message.guild.id}.json`)
    .then(function (response) {
      if (
        response.data == null ||
        response.data.guild_settings.group_id == -1
      ) {
        setup = false;
      }
    });
  ...
  if (setup == false) {
    ...
    if (!args[1]) {
      var badEmbed = new Discord.MessageEmbed()
        .setColor(0xf54242)
        .setDescription(
          `Sorry ${message.author}, but you must provide me with the group's ID!\n\n**Example - ${client.config.prefix}bind 5940055**`
        );
      return message.channel.send(badEmbed);
    }
    ...
    // fetch data
    await axios
      .get(`https://api.roblox.com/groups/${args[1]}`)
      .then(function (response) {
    ...
    if (groupID == 0) {
      var badEmbed = new Discord.MessageEmbed()
        .setColor(0xf54242)
        .setDescription(
          `Sorry ${message.author}, but that wasn't a valid group ID!`
        );
      return message.channel.send(badEmbed);
    }
    ...
    for (i = 0; i < roles.length; i++) {
      ...
      if (i == 0) {
        var doneEmbed = new Discord.MessageEmbed()
          .setColor(0x21ff7a)
          .setDescription(
            `**:white_check_mark: Rank: ${roles[i].Name} | Required ${client.config.experience_name}: 0 :white_check_mark: **\n\nThe first rank is automatically set to 0 ${client.config.experience_name}`
          );
      ...
      if (i == roles.length - 1) {
        var doneEmbed = new Discord.MessageEmbed()
          .setColor(0x21ff7a)
          .setDescription(
            `**:white_check_mark: Rank: ${roles[i].Name} | Required ${client.config.experience_name}: -1 :white_check_mark: **\n\nThe last rank, ${roles[i].Name}, is not reachable via promotions`
          );
      ...
      // other ranks
      const location = await message.author
        .send({
          embed: {
        ...
        .catch(() => {
          return message.channel.send(
            `Sorry ${message.author}, but I couldn't direct message you!`
          );
        });

      // collection
      const timeCollectionThing = { max: 1, time: 30000, errors: ["time"] };
      const collected = await location
        .awaitMessages(
          (response) => message.author === response.author,
          timeCollectionThing
        )
        .catch(() => null);
      ...
      if (!collected) {
        return message.channel.send(
          `Sorry ${message.author}, but I've waited too long for a response from you--please try again later!`
        );
      }
      ...
      if (
        isNaN(Number(responseArray[0])) ||
        Number(responseArray[0] < -1) ||
        (Number(responseArray[0]) < previous_number &&
          Number(responseArray[0]) !== -1)
      ) {
        return message.author.send(
          `Sorry ${message.author}, but you didn't provide me with a numerical number greater than 0 (or greater than the previous valid number: ${previous_number})!\n**I've cancelled this setup, please try again!**`
        );
      } else {
        ...
        var doneEmbed = new Discord.MessageEmbed()
          .setColor(0x21ff7a)
          .setDescription(
            `**:white_check_mark: Rank: ${roles[i].Name} | Required ${
              client.config.experience_name
            }: ${Number(responseArray[0])} :white_check_mark: **`
          );
    ...
    for (i = 0; i < numbers.length; i++) {
      if (numbers[i] == -1) {
        ...
        doneEmbed.addField(
          `**\`${roles[i].Name}\` - \`(${roles[i].Rank})\`**`,
          `**${Number(numbers[i])} ${client.config.experience_name}**`,
          true
        );
      }
    }
